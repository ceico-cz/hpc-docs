...
CALLOUT = {"is-info": ("info", "Note"), "is-warning": ("warning", "Important"),
           "is-success": ("success", "Tip"), "is-danger": ("danger", "Warning")}
# pages redesigned by hand after the import; not overwritten on a re-sync
HAND_EDITED = {"home"}
...
class Converter:
        ...
        while i < len(lines):
            ...
            if line.startswith(">"):
                ...
                if m:
                    ...
                    i += 1
                if kind in CALLOUT:
                    emit(f'!!! {CALLOUT[kind][0]} "{CALLOUT[kind][1]}"')
                    for b in block:
    ...
    def page(self, p):
        ...
        fm.append(f"wikijs_updated: {p['updatedAt'][:10]}")
...
def main():
    ...
    ap.add_argument("--write-nav", action="store_true", help="regenerate nav.yml from the Wiki.js navigation")
    a = ap.parse_args()
    ...
    for p in pages:
        ...
        if p["path"] in HAND_EDITED and os.path.exists(dest):
            print(f"keep  {p['path']} (hand-edited)", file=sys.stderr)
            conv.page(p)  # still collect the assets it references
            continue
